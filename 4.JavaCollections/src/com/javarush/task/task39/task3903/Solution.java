...
public class Solution {
    ...
    public static long swapBits(long number, int i, int j) {
        String numL = Long.toBinaryString(number);
        System.out.println(numL);
        char[] chars = numL.toCharArray();
        char swap = chars[chars.length - 1 - i];
        char swap2 = chars[chars.length - 1 - j];
        chars[chars.length - 1 - j] = swap;
        chars[chars.length - 1 - i] = swap2;
